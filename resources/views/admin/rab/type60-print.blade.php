...
<head>
    ...
    <title>RAB Type {{ $selectedType->nama ?? '60' }} - {{ $selectedLocation->nama ?? '' }} - Unit {{ $selectedUnit->kode_unit ?? '' }}</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <a href="{{ route('rab.type60', ['type_id' => $selectedType->id ?? '', 'unit_id' => $selectedUnit->id ?? '', 'location_id' => $selectedLocation->id ?? '']) }}" class="back-btn no-print">ΓåÉ Kembali</a>
