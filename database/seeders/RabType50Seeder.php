...
class RabType50Seeder extends Seeder
{
    public function run()
    {
        ...
        // Ambil categories untuk mapping
        $categories = \App\Models\RabCategory::pluck('id', 'kode')->toArray();

        // -----------------------------------------------
        // 1. DAFTAR BAHAN BAKU TYPE 50 (dengan kategori)
        // Format: ['kategori' => ['item_name' => qty]]
        // -----------------------------------------------
        $values = [
            ...
            'E' => [
                'Papan' => 24,
                'Kayu 3/5' => 25,
                'Besi ø6' => 5, // Catatan: User ada 3 "Besi ø6" dengan nilai 5, 2, 5 - hanya bisa 1 nilai per kategori
                'Besi ø8' => 8,
            ...
            'H' => [
                ...
                'Seng Metal 2 Susun (hitam)' => 0, // User: kosong = 0
                'Perabung Hitam' => 0, // User: kosong = 0
                'Perabung Merah' => 1.5,
                'Paku 2 inch' => 1,
                'Paku 3 inch' => 1,
            ],
        ...
        // ----------------------------------------------------
        // 2. MATCH KE TEMPLATE & INSERT (dengan kategori)
        // ----------------------------------------------------
        foreach ($values as $categoryCode => $items) {
            $categoryId = $categories[$categoryCode] ?? null;
            
            if (!$categoryId) {
                echo "⚠ KATEGORI TIDAK DITEMUKAN: $categoryCode\n";
                continue;
            }

            foreach ($items as $itemName => $qty) {
                // Cari template berdasarkan kategori dan item_name
                $tpl = RabTemplate::where('category_id', $categoryId)
                    ->where('item_name', $itemName)
                    ->first();

                if (!$tpl) {
                    echo "⚠ ITEM TIDAK DITEMUKAN DI TEMPLATE [$categoryCode]: $itemName\n";
                    continue;
                }
